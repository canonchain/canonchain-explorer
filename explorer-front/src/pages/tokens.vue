        ...
        <div class="page-container">
            <div class="container">
                <div class="list-wrap" v-loading="loadingSwitch">
                    <strong class="list-title">{{ $t('token.token_list') }}</strong>
                    <span class="sub_header-des">{{ $t('normal_trans.total') }} {{ TOTAL_VAL }} 个</span>
    ...
    methods: {
        async getPaginationFlag(val) {
            ...
            if (val == "left") {
                //取第一个item
                self.$router.push(
                    `/normal_trans?token_id=${self.pageFirstItem.token_id}&position=2`
                );
                return;
            }
            ...
            if (val == "right") {
                //取最后一个item
                self.$router.push(
                    `/normal_trans?token_id=${self.pageLastItem.token_id}&position=3`
                );
                return;
            }
        },
